...
def memory_plot(df):
    ...
    p.save("doubly-linked-list-memory-reasoning.png",
        dpi=300, width=PLOT_WIDTH, height=PLOT_HEIGHT,
        units=PLOT_SIZE_UNIT)
    p.save("doubly-linked-list-memory-reasoning.pdf",
        dpi=300, width=PLOT_WIDTH, height=PLOT_HEIGHT,
        units=PLOT_SIZE_UNIT)
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("doubly-linked-list-repeat.csv", header=None)
    ...
    df_tools = ['F*', 'Dafny', 'Creusot', 'Verus']
